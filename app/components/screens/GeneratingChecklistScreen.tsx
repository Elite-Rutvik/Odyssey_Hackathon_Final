...
const ReportView: React.FC<{ content: string }> = ({ content }) => {
  const formatContent = (text: string) => {
    ...
    const sections = cleanText.split(
      /(?=DOCUMENT FORMAT|REQUIRED ATTACHMENTS|SUBMISSION FORMAT|ADDITIONAL REQUIREMENTS)/g
    );
      ...
      <div class="space-y-6 text-black">
        ${sections
          ...
          .map((section) => {
            ...
            // Special handling for DOCUMENT FORMAT REQUIREMENTS section
            if (title.includes("DOCUMENT FORMAT")) {
              return `
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  <h3 class="font-bold text-lg text-black border-b border-gray-200 pb-2 mb-4">${title.trim()}</h3>
                  <p class="text-black leading-relaxed">
                    ${content.join(" ").replace(/\*/g, "").trim()}
                  </p>
                </div>
              `;
            }

            // Regular list format for other sections
            return `
              <div class="bg-white p-4 rounded-lg shadow-sm">
                <h3 class="font-bold text-lg text-black border-b border-gray-200 pb-2 mb-4">${title.trim()}</h3>
                <ul class="space-y-2">
...
const GeneratingChecklistScreen: React.FC = () => {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-black text-lg mb-6">
          Generate and manage proposal checklists to ensure all requirements are
          met
        </p>
...
export default GeneratingChecklistScreen;
